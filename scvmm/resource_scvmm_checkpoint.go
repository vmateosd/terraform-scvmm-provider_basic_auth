...
func resourceCheckpointAdd(d *schema.ResourceData, meta interface{}) error {
	...
	validationArguments := " " + timeout + " \"" + vmmServer + "\" \"" + virtualMachineName + "\" \"" + checkpointName + "\""
	...
	arguments := " " + timeout + " \"" + virtualMachineName + "\" \"" + vmmServer + "\" \"" + checkpointName + "\""
	...
	if err == "" {
		terraformID := vmmServer + "_" + virtualMachineName + "_" + checkpointName
		d.SetId(terraformID)
	}
	return nil
}
...
func resourceCheckpointRead(d *schema.ResourceData, meta interface{}) error {
	...
	arguments := " " + timeout + " \"" + vmmServer + "\" \"" + virtualMachineName + "\" \"" + checkpointName + "\""
	_, err := execScript(connection, script, filename, arguments)
	if err != "" {
...
func resourceCheckpointDelete(d *schema.ResourceData, meta interface{}) error {
	...
	log.Println("[DEBUG] Checking if Checkpoint Exists")
	...
	log.Println("[DEBUG] Checkpoint Found")
