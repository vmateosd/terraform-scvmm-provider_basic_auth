...
func resourceCreateVirtualDisk(d *schema.ResourceData, meta interface{}) error {
	...
	filename := "createVD"
	arguments := timeout + " \"" + vmmServer + "\" \"" + virtualMachineName + "\" \"" + volumeName + "\" " + volumeSize
	...
	if err == "" {
		terraformID := vmmServer + "_" + virtualMachineName + "_" + volumeName
		d.SetId(terraformID)
	}
	return nil
}
...
func resourceReadVirtualDisk(d *schema.ResourceData, meta interface{}) error {
	...
	filename := "readVD"
	arguments := timeout + " \"" + vmmServer + "\" \"" + virtualMachineName + "\" \"" + volumeName + "\""
	_, err := execScript(connection, script, filename, arguments)
	if err != "" {
...
func resourceDeleteVirtualDisk(d *schema.ResourceData, meta interface{}) error {
	log.Println("[DEBUG] Checking if Virtual Disk Exists")
	...
	log.Println("[DEBUG] Virtual Disk Found")
	connection := meta.(*winrm.Client)
	virtualMachineName := d.Get("vm_name").(string)
	vmmServer := d.Get("vmm_server").(string)
	volumeName := d.Get("virtual_disk_name").(string)
