...
func execScript(client *winrm.Client, script string, fileName string, arguments string) (string, string) {
	...
	outputFunc := func(input string) {
		log.Println("[DEBUG] Output received \n" + input)
		output = input
	}
	errorFunc := func(input string) {
		log.Println("[DEBUG] Error Output received \n" + input)
		errorOutput = input
	}
	...
	log.Println("[DEBUG] Shell script: " + file)
	log.Println("[DEBUG] Arguments: " + arguments)
	...
	log.Println("[DEBUG] Waiting for commands to be executed.")
	cmd.Wait()
	wg.Wait()
	shell.Close()
	log.Println("[DEBUG] winrm Shell closed")
