...
func resourceSCVMMVirtualMachine() *schema.Resource {
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vm_name": &schema.Schema{
				...
				Required:     true,
				ForceNew:     true,
				ValidateFunc: validateVirtualMachineName,
			},
...
func resourceVirtualMachineCreate(d *schema.ResourceData, metadata interface{}) error {
	...
	log.Println("[DEBUG] Validation successful")
	...
	filename := "createVM"
	arguments := " \"" + templateName + "\" \"" + vmmServer + "\" \"" + cloudName + "\" \"" + virtualMachineName + "\""
	...
	if err == "" {
		terraformID := vmmServer + "_" + cloudName + "_" + virtualMachineName
		d.SetId(terraformID)
	}
...
func resourceVirtualMachineRead(d *schema.ResourceData, metadata interface{}) error {
	...
	filename := "readVM"
	arguments := timeout + " \"" + vmmServer + "\" \"" + virtualMachineName + "\""
	_, err := execScript(connection, script, filename, arguments)
...
func resourceVirtualMachineDelete(d *schema.ResourceData, metadata interface{}) error {
	log.Println("[DEBUG] Checking if VM Exists")
	resourceVirtualMachineRead(d, metadata)
	if d.Id() == "" {
		log.Println("[Error] VM is not available")
		return fmt.Errorf("[Error] VM does not Exist")
	}
	log.Println("[DEBUG] VM Found")
	connection := metadata.(*winrm.Client)
	vmmServer := d.Get("vmm_server").(string)
	virtualMachineName := d.Get("vm_name").(string)
	timeout := d.Get("timeout").(string)
	...
	filename := "deleteVM"
	arguments := timeout + " \"" + virtualMachineName + "\" \"" + vmmServer + "\""
	_, err := execScript(connection, script, filename, arguments)
